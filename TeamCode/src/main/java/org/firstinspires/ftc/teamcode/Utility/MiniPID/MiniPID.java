 ...
 * MiniPID pid = new MiniPID(p,i,d); <br> ...looping code...{ <br> output=
 * pid.getOutput(sensorvalue,target); <br> }
 *
...
public class MiniPID {
  ...
  //**********************************
  // ROB NOTE: MADE ALL PROTECTED FOR EASE OF CHANGE IN MINIPIDEX
  protected double P;
  protected double I;
  protected double D;
  protected double F;
  ...
  //**********************************
  // ROB NOTE: Made all protected protected for use in EX
   ...
   * controller always gets to the control mode. Typically tuned second for "Position" based modes,
   * and third for "Rate" or continuous based modes. <br> Affects output through
   * <b>output+=previous_errors*Igain ;previous_errors+=current_error</b>
   *
   * @param i New gain value for the Integral term
   * @see MiniPID#setMaxIOutput(double) for how to restrict
   */
   ...
   * <b>[-output, output]</b>
   *
   * @param output +- max/min for output
   */
   ...
   *
   * @param setpoint target for PID system
   * @see MiniPID#getOutput(double actual) <br>
   */
   ...
   * value. Larger values use historical data more heavily, with low values weigh newer data. 0 will
   * disable, filtering, and use only the most recent value. <br> Increasing the filter strength
   * will P and D oscillations, but force larger I values and increase I term overshoot.<br> Uses an
   ...
   */
  protected boolean bounded(double value, double min, double max) {
